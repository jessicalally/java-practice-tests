...
public class DocFileUtils {
  ...
  public static int getTotalDirectorySize(DocDirectory directory) {
    ...
    int size = directory.getDirectories().stream().map(DocFileUtils::getTotalDirectorySize)
        .reduce(directory.getSize(), Integer::sum);
    return size + directory.getDataFiles().stream().map(DocDataFile::getSize)
  ...
  public static Optional<DocDataFile> searchForByte(DocFile root, byte someByte) {
    // TODO: implement as part of Question 4
    if (root.isDataFile()) {
      DocDataFile dataFile = (DocDataFile) root;
      ...
      DocDirectory directory = (DocDirectory) root;
      return directory.getAllFiles().stream()
